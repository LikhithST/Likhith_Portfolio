import { B as e, C as t, V as n, W as r, _ as i, a, b as o, d as s, q as c, s as l, v as u, y as d } from "./chunk-ICPOFSXX-cosQBm9P.js";
import { g as f, h as p, p as m } from "./src-DiRGiyL4.js";
import "./dist-BhM2qfql.js";
import { g as h, i as g, m as _ } from "./chunk-5PVQY5BW-BPITZuxu.js";
...
import { t as v } from "./chunk-4BX2VUAB-Zt4DWBfT.js";
import { t as y } from "./mermaid-parser.core-BhyjZala.js";
import { t as b } from "./chunk-QZHKN3VN-CoRl2cAx.js";
var x = { NORMAL: 0, REVERSE: 1, HIGHLIGHT: 2, MERGE: 3, CHERRY_PICK: 4 }, ee = s.gitGraph, S = p(() => g({ ...ee, ...d().gitGraph }), `getConfig`), C = new b(() => {
...
p(T, `uniqBy`);
var te = p(function(e2) {
  C.records.direction = e2;
}, `setDirection`), ne = p(function(e2) {
...
}, `setOptions`), re = p(function() {
  return C.records.options;
}, `getOptions`), ie = p(function(e2) {
  let t2 = e2.msg, n2 = e2.id, r2 = e2.type, i2 = e2.tags;
  f.info(`commit`, t2, n2, r2, i2), f.debug(`Entering commit:`, t2, n2, r2, i2);
  let a2 = S();
  n2 = l.sanitizeText(n2, a2), t2 = l.sanitizeText(t2, a2), i2 = i2 == null ? void 0 : i2.map((e3) => l.sanitizeText(e3, a2));
  ...
  C.records.head = o2, f.info(`main branch`, a2.mainBranchName), C.records.commits.has(o2.id) && f.warn(`Commit ID ${o2.id} already exists`), C.records.commits.set(o2.id, o2), C.records.branches.set(C.records.currBranch, o2.id), f.debug(`in pushCommit ` + o2.id);
}, `commit`), ae = p(function(e2) {
  let t2 = e2.name, n2 = e2.order;
  if (t2 = l.sanitizeText(t2, S()), C.records.branches.has(t2)) throw Error(`Trying to create an existing branch. (Help: Either use a new name if you want create a new branch or try using "checkout ${t2}")`);
  C.records.branches.set(t2, C.records.head == null ? null : C.records.head.id), C.records.branchConfig.set(t2, { name: t2, order: n2 }), E(t2), f.debug(`in createBranch`);
}, `branch`), oe = p((e2) => {
  let t2 = e2.branch, n2 = e2.id, r2 = e2.type, i2 = e2.tags, a2 = S();
  t2 = l.sanitizeText(t2, a2), n2 && (n2 = l.sanitizeText(n2, a2));
  ...
  let d2 = s2 || ``, p2 = { id: n2 || `${C.records.seq}-${w()}`, message: `merged branch ${t2} into ${C.records.currBranch}`, seq: C.records.seq++, parents: C.records.head == null ? [] : [C.records.head.id, d2], branch: C.records.currBranch, type: x.MERGE, customType: r2, customId: !!n2, tags: i2 ?? [] };
  C.records.head = p2, C.records.commits.set(p2.id, p2), C.records.branches.set(C.records.currBranch, p2.id), f.debug(C.records.branches), f.debug(`in mergeBranch`);
}, `merge`), se = p(function(e2) {
  let t2 = e2.id, n2 = e2.targetId, r2 = e2.tags, i2 = e2.parent;
  f.debug(`Entering cherryPick:`, t2, n2, r2);
  let a2 = S();
  if (t2 = l.sanitizeText(t2, a2), n2 = l.sanitizeText(n2, a2), r2 = r2 == null ? void 0 : r2.map((e3) => l.sanitizeText(e3, a2)), i2 = l.sanitizeText(i2, a2), !t2 || !C.records.commits.has(t2)) {
  ...
  if (!n2 || !C.records.commits.has(n2)) {
    ...
    C.records.head = c2, C.records.commits.set(c2.id, c2), C.records.branches.set(C.records.currBranch, c2.id), f.debug(C.records.branches), f.debug(`in cherryPick`);
  }
}, `cherryPick`), E = p(function(e2) {
  if (e2 = l.sanitizeText(e2, S()), C.records.branches.has(e2)) {
...
var ce = p(function() {
...
}, `getBranchesAsObjArray`), de = p(function() {
  return C.records.branches;
}, `getBranches`), fe = p(function() {
  return C.records.commits;
}, `getCommits`), k = p(function() {
  let e2 = [...C.records.commits.values()];
  return e2.forEach(function(e3) {
    f.debug(e3.id);
  }), e2.sort((e3, t2) => e3.seq - t2.seq), e2;
}, `getCommitsArray`), A = { commitType: x, getConfig: S, setDirection: te, setOptions: ne, getOptions: re, commit: ie, branch: ae, merge: oe, cherryPick: se, checkout: E, prettyPrint: ce, clear: le, getBranchesAsObjArray: ue, getBranches: de, getCommits: fe, getCommitsArray: k, getCurrentBranch: p(function() {
...
}, `getHead`), setAccTitle: n, getAccTitle: u, getAccDescription: i, setAccDescription: e, setDiagramTitle: r, getDiagramTitle: t }, pe = p((e2, t2) => {
  v(e2, t2), e2.dir && t2.setDirection(e2.dir);
  for (let n2 of e2.statements) me(n2, t2);
}, `populate`), me = p((e2, t2) => {
  let n2 = { Commit: p((e3) => t2.commit(he(e3)), `Commit`), Branch: p((e3) => t2.branch(ge(e3)), `Branch`), Merge: p((e3) => t2.merge(_e(e3)), `Merge`), Checkout: p((e3) => t2.checkout(ve(e3)), `Checkout`), CherryPicking: p((e3) => t2.cherryPick(ye(e3)), `CherryPicking`) }[e2.$type];
  n2 ? n2(e2) : f.error(`Unknown statement type: ${e2.$type}`);
}, `parseStatement`), he = p((e2) => ({ id: e2.id, msg: e2.message ?? ``, type: e2.type === void 0 ? x.NORMAL : x[e2.type], tags: e2.tags ?? void 0 }), `parseCommit`), ge = p((e2) => ({ name: e2.name, order: e2.order ?? 0 }), `parseBranch`), _e = p((e2) => ({ branch: e2.branch, id: e2.id ?? ``, type: e2.type === void 0 ? void 0 : x[e2.type], tags: e2.tags ?? void 0 }), `parseMerge`), ve = p((e2) => e2.branch, `parseCheckout`), ye = p((e2) => {
  var _a;
  return { id: e2.id, targetId: ``, tags: ((_a = e2.tags) == null ? void 0 : _a.length) === 0 ? void 0 : e2.tags, parent: e2.parent };
}, `parseCherryPicking`), be = { parse: p(async (e2) => {
  let t2 = await y(`gitGraph`, e2);
  f.debug(t2), pe(t2, A);
}, `parse`) }, j = 10, M = 40, N = 4, P = 2, F = 8, I = /* @__PURE__ */ new Set([`redux`, `redux-dark`, `redux-color`, `redux-dark-color`]), L = 12, R = /* @__PURE__ */ new Set([`redux-color`, `redux-dark-color`]), xe = /* @__PURE__ */ new Set([`dark`, `redux-dark`, `redux-dark-color`, `neo-dark`]), z = p((e2, t2, n2 = false) => n2 && e2 > 0 ? (e2 - 1) % (t2 - 1) + 1 : e2 % t2, `calcColorIndex`), B = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), H = 30, U = /* @__PURE__ */ new Map(), W = [], G = 0, K = `LR`, q = p(() => {
  ...
  let t2, n2, r2;
  return K === `BT` ? (n2 = p((e3, t3) => e3 <= t3, `comparisonFunc`), r2 = 1 / 0) : (n2 = p((e3, t3) => e3 >= t3, `comparisonFunc`), r2 = 0), e2.forEach((e3) => {
    var _a, _b;
    let i2 = K === `TB` || K == `BT` ? (_a = V.get(e3)) == null ? void 0 : _a.y : (_b = V.get(e3)) == null ? void 0 : _b.x;
    i2 !== void 0 && n2(i2, r2) && (t2 = e3, r2 = i2);
  }), t2;
}, `findClosestParent`), Se = p((e2) => {
  let t2 = ``, n2 = 1 / 0;
  return e2.forEach((e3) => {
    let r2 = V.get(e3).y;
    r2 <= n2 && (t2 = e3, n2 = r2);
  }), t2 || void 0;
}, `findClosestParentBT`), Ce = p((e2, t2, n2) => {
  let r2 = n2, i2 = n2, a2 = [];
  e2.forEach((e3) => {
    let n3 = t2.get(e3);
  ...
  return n2;
}, `findClosestParentPos`), Te = p((e2) => we(e2) + M, `calculateCommitPosition`), Ee = p((e2, t2) => {
...
}, `setCommitPosition`), De = p((e2, t2, n2) => {
  let r2 = B.get(e2.branch);
  if (!r2) throw Error(`Branch not found for commit ${e2.id}`);
  let i2 = t2 + n2, a2 = r2.pos;
  V.set(e2.id, { x: a2, y: i2 });
}, `setRootPosition`), Oe = p((e2, t2, n2, r2, i2, a2) => {
  let { theme: s2 } = o(), c2 = I.has(s2 ?? ``), l2 = R.has(s2 ?? ``), u2 = xe.has(s2 ?? ``);
  if (a2 === x.HIGHLIGHT) e2.append(`rect`).attr(`x`, n2.x - 10 + (c2 ? 3 : 0)).attr(`y`, n2.y - 10 + (c2 ? 3 : 0)).attr(`width`, c2 ? 14 : 20).attr(`height`, c2 ? 14 : 20).attr(`class`, `commit ${t2.id} commit-highlight${z(i2, F, l2)} ${r2}-outer`), e2.append(`rect`).attr(`x`, n2.x - 6 + (c2 ? 2 : 0)).attr(`y`, n2.y - 6 + (c2 ? 2 : 0)).attr(`width`, c2 ? 8 : 12).attr(`height`, c2 ? 8 : 12).attr(`class`, `commit ${t2.id} commit${z(i2, F, l2)} ${r2}-inner`);
  else if (a2 === x.CHERRY_PICK) e2.append(`circle`).attr(`cx`, n2.x).attr(`cy`, n2.y).attr(`r`, c2 ? 7 : 10).attr(`class`, `commit ${t2.id} ${r2}`), e2.append(`circle`).attr(`cx`, n2.x - 3).attr(`cy`, n2.y + 2).attr(`r`, c2 ? 2.5 : 2.75).attr(`fill`, u2 ? `#000000` : `#fff`).attr(`class`, `commit ${t2.id} ${r2}`), e2.append(`circle`).attr(`cx`, n2.x + 3).attr(`cy`, n2.y + 2).attr(`r`, c2 ? 2.5 : 2.75).attr(`fill`, u2 ? `#000000` : `#fff`).attr(`class`, `commit ${t2.id} ${r2}`), e2.append(`line`).attr(`x1`, n2.x + 3).attr(`y1`, n2.y + 1).attr(`x2`, n2.x).attr(`y2`, n2.y - 5).attr(`stroke`, u2 ? `#000000` : `#fff`).attr(`class`, `commit ${t2.id} ${r2}`), e2.append(`line`).attr(`x1`, n2.x - 3).attr(`y1`, n2.y + 1).attr(`x2`, n2.x).attr(`y2`, n2.y - 5).attr(`stroke`, u2 ? `#000000` : `#fff`).attr(`class`, `commit ${t2.id} ${r2}`);
  else {
    let o2 = e2.append(`circle`);
    if (o2.attr(`cx`, n2.x), o2.attr(`cy`, n2.y), o2.attr(`r`, c2 ? 7 : 10), o2.attr(`class`, `commit ${t2.id} commit${z(i2, F, l2)}`), a2 === x.MERGE) {
      let a3 = e2.append(`circle`);
      a3.attr(`cx`, n2.x), a3.attr(`cy`, n2.y), a3.attr(`r`, c2 ? 5 : 6), a3.attr(`class`, `commit ${r2} ${t2.id} commit${z(i2, F, l2)}`);
    }
    if (a2 === x.REVERSE) {
      let a3 = e2.append(`path`), o3 = c2 ? 4 : 5;
      a3.attr(`d`, `M ${n2.x - o3},${n2.y - o3}L${n2.x + o3},${n2.y + o3}M${n2.x - o3},${n2.y + o3}L${n2.x + o3},${n2.y - o3}`).attr(`class`, `commit ${r2} ${t2.id} commit${z(i2, F, l2)}`);
    }
  }
}, `drawCommitBullet`), ke = p((e2, t2, n2, r2, i2) => {
  var _a;
...
}, `drawCommitLabel`), Ae = p((e2, t2, n2, r2) => {
  var _a;
  if (t2.tags.length > 0) {
...
}, `getCommitClassType`), Me = p((e2, t2, n2, r2) => {
  let i2 = { x: 0, y: 0 };
  ...
  return 0;
}, `calculatePosition`), Ne = p((e2, t2, n2) => {
  var _a, _b;
  ...
  let s2 = I.has(o().theme ?? ``);
  return { x: a2, y: K === `TB` || K === `BT` ? r2 : i2 + (s2 ? L / 2 + 1 : -2), posWithOffset: r2 };
}, `getCommitPosition`), X = p((e2, t2, n2, r2) => {
  let i2 = e2.append(`g`).attr(`class`, `commit-bullets`), a2 = e2.append(`g`).attr(`class`, `commit-labels`), o2 = K === `TB` || K === `BT` ? H : 0, s2 = [...t2.keys()], c2 = r2.parallelCommits ?? false, l2 = p((e3, n3) => {
    var _a, _b;
    let r3 = (_a = t2.get(e3)) == null ? void 0 : _a.seq, i3 = (_b = t2.get(n3)) == null ? void 0 : _b.seq;
    return r3 !== void 0 && i3 !== void 0 ? r3 - i3 : 0;
  }, `sortKeys`), u2 = s2.sort(l2);
  K === `BT` && (c2 && Ce(u2, t2, o2), u2 = u2.reverse()), u2.forEach((e3) => {
    var _a;
    let s3 = t2.get(e3);
    if (!s3) throw Error(`Commit not found for key ${e3}`);
    c2 && (o2 = Me(s3, K, o2, V));
    let l3 = Ne(s3, o2, c2);
    if (n2) {
      let e4 = je(s3), t3 = s3.customType ?? s3.type;
      Oe(i2, s3, l3, e4, ((_a = B.get(s3.branch)) == null ? void 0 : _a.index) ?? 0, t3), ke(a2, s3, l3, o2, r2), Ae(a2, s3, l3, o2);
    }
    K === `TB` || K === `BT` ? V.set(s3.id, { x: l3.x, y: l3.posWithOffset }) : V.set(s3.id, { x: l3.posWithOffset, y: l3.y }), o2 = K === `BT` && c2 ? o2 + M : o2 + M + j, o2 > G && (G = o2);
  });
}, `drawCommits`), Pe = p((e2, t2, n2, r2, i2) => {
  let a2 = (K === `TB` || K === `BT` ? n2.x < r2.x : n2.y < r2.y) ? t2.branch : e2.branch, o2 = p((e3) => e3.branch === a2, `isOnBranchToGetCurve`), s2 = p((n3) => n3.seq > e2.seq && n3.seq < t2.seq, `isBetweenCommits`);
  return [...i2.values()].some((e3) => s2(e3) && o2(e3));
}, `shouldRerouteArrow`), Z = p((e2, t2, n2 = 0) => {
  let r2 = e2 + Math.abs(e2 - t2) / 2;
  return n2 > 5 ? r2 : W.every((e3) => Math.abs(e3 - r2) >= 10) ? (W.push(r2), r2) : Z(e2, t2 - Math.abs(e2 - t2) / 5, n2 + 1);
}, `findLane`), Fe = p((e2, t2, n2, r2) => {
  var _a, _b, _c, _d, _e2;
  let { theme: i2 } = o(), a2 = R.has(i2 ?? ``), s2 = V.get(t2.id), c2 = V.get(n2.id);
  if (s2 === void 0 || c2 === void 0) throw Error(`Commit positions not found for commits ${t2.id} and ${n2.id}`);
  let l2 = Pe(t2, n2, s2, c2, r2), u2 = ``, d2 = ``, f2 = 0, p2 = 0, m2 = (_a = B.get(n2.branch)) == null ? void 0 : _a.index;
  ...
  if (l2) {
    ...
    let e3 = s2.y < c2.y ? Z(s2.y, c2.y) : Z(c2.y, s2.y), n3 = s2.x < c2.x ? Z(s2.x, c2.x) : Z(c2.x, s2.x);
    K === `TB` ? s2.x < c2.x ? h2 = `M ${s2.x} ${s2.y} L ${n3 - f2} ${s2.y} ${d2} ${n3} ${s2.y + p2} L ${n3} ${c2.y - f2} ${u2} ${n3 + p2} ${c2.y} L ${c2.x} ${c2.y}` : (m2 = (_c = B.get(t2.branch)) == null ? void 0 : _c.index, h2 = `M ${s2.x} ${s2.y} L ${n3 + f2} ${s2.y} ${u2} ${n3} ${s2.y + p2} L ${n3} ${c2.y - f2} ${d2} ${n3 - p2} ${c2.y} L ${c2.x} ${c2.y}`) : K === `BT` ? s2.x < c2.x ? h2 = `M ${s2.x} ${s2.y} L ${n3 - f2} ${s2.y} ${u2} ${n3} ${s2.y - p2} L ${n3} ${c2.y + f2} ${d2} ${n3 + p2} ${c2.y} L ${c2.x} ${c2.y}` : (m2 = (_d = B.get(t2.branch)) == null ? void 0 : _d.index, h2 = `M ${s2.x} ${s2.y} L ${n3 + f2} ${s2.y} ${d2} ${n3} ${s2.y - p2} L ${n3} ${c2.y + f2} ${u2} ${n3 - p2} ${c2.y} L ${c2.x} ${c2.y}`) : s2.y < c2.y ? h2 = `M ${s2.x} ${s2.y} L ${s2.x} ${e3 - f2} ${u2} ${s2.x + p2} ${e3} L ${c2.x - f2} ${e3} ${d2} ${c2.x} ${e3 + p2} L ${c2.x} ${c2.y}` : (m2 = (_e2 = B.get(t2.branch)) == null ? void 0 : _e2.index, h2 = `M ${s2.x} ${s2.y} L ${s2.x} ${e3 + f2} ${d2} ${s2.x + p2} ${e3} L ${c2.x - f2} ${e3} ${u2} ${c2.x} ${e3 - p2} L ${c2.x} ${c2.y}`);
  } else u2 = `A 20 20, 0, 0, 0,`, d2 = `A 20 20, 0, 0, 1,`, f2 = 20, p2 = 20, K === `TB` ? (s2.x < c2.x && (h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y - f2} ${u2} ${s2.x + p2} ${c2.y} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${c2.x - f2} ${s2.y} ${d2} ${c2.x} ${s2.y + p2} L ${c2.x} ${c2.y}`), s2.x > c2.x && (u2 = `A 20 20, 0, 0, 0,`, d2 = `A 20 20, 0, 0, 1,`, f2 = 20, p2 = 20, h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y - f2} ${d2} ${s2.x - p2} ${c2.y} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${c2.x + f2} ${s2.y} ${u2} ${c2.x} ${s2.y + p2} L ${c2.x} ${c2.y}`), s2.x === c2.x && (h2 = `M ${s2.x} ${s2.y} L ${c2.x} ${c2.y}`)) : K === `BT` ? (s2.x < c2.x && (h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y + f2} ${d2} ${s2.x + p2} ${c2.y} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${c2.x - f2} ${s2.y} ${u2} ${c2.x} ${s2.y - p2} L ${c2.x} ${c2.y}`), s2.x > c2.x && (u2 = `A 20 20, 0, 0, 0,`, d2 = `A 20 20, 0, 0, 1,`, f2 = 20, p2 = 20, h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y + f2} ${u2} ${s2.x - p2} ${c2.y} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${c2.x + f2} ${s2.y} ${d2} ${c2.x} ${s2.y - p2} L ${c2.x} ${c2.y}`), s2.x === c2.x && (h2 = `M ${s2.x} ${s2.y} L ${c2.x} ${c2.y}`)) : (s2.y < c2.y && (h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${c2.x - f2} ${s2.y} ${d2} ${c2.x} ${s2.y + p2} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y - f2} ${u2} ${s2.x + p2} ${c2.y} L ${c2.x} ${c2.y}`), s2.y > c2.y && (h2 = n2.type === x.MERGE && t2.id !== n2.parents[0] ? `M ${s2.x} ${s2.y} L ${c2.x - f2} ${s2.y} ${u2} ${c2.x} ${s2.y - p2} L ${c2.x} ${c2.y}` : `M ${s2.x} ${s2.y} L ${s2.x} ${c2.y + f2} ${d2} ${s2.x + p2} ${c2.y} L ${c2.x} ${c2.y}`), s2.y === c2.y && (h2 = `M ${s2.x} ${s2.y} L ${c2.x} ${c2.y}`));
  if (h2 === void 0) throw Error(`Line definition not found`);
  e2.append(`path`).attr(`d`, h2).attr(`class`, `arrow arrow` + z(m2, F, a2));
}, `drawArrow`), Ie = p((e2, t2) => {
...
}, `drawArrows`), Le = p((e2, t2, n2, r2) => {
  let { look: i2, theme: a2, themeVariables: s2 } = o(), { dropShadow: c2, THEME_COLOR_LIMIT: l2 } = s2, u2 = I.has(a2 ?? ``), d2 = R.has(a2 ?? ``), f2 = e2.append(`g`);
  t2.forEach((e3, t3) => {
    var _a;
    let a3 = z(t3, u2 ? l2 : F, d2), o2 = (_a = B.get(e3.name)) == null ? void 0 : _a.pos;
    if (o2 === void 0) throw Error(`Position not found for branch ${e3.name}`);
    let s3 = K === `TB` || K === `BT` ? o2 : u2 ? o2 + L / 2 + 1 : o2 - 2, p2 = f2.append(`line`);
    p2.attr(`x1`, 0), p2.attr(`y1`, s3), p2.attr(`x2`, G), p2.attr(`y2`, s3), p2.attr(`class`, `branch branch` + a3), K === `TB` ? (p2.attr(`y1`, H), p2.attr(`x1`, o2), p2.attr(`y2`, G), p2.attr(`x2`, o2)) : K === `BT` && (p2.attr(`y1`, G), p2.attr(`x1`, o2), p2.attr(`y2`, H), p2.attr(`x2`, o2)), W.push(s3);
    ...
    i2 === `neo` && g2.attr(`data-look`, `neo`), g2.attr(`class`, `branchLabelBkg label` + a3).attr(`style`, i2 === `neo` ? `filter:${u2 ? `url(#${r2}-drop-shadow)` : c2}` : ``).attr(`rx`, y2).attr(`ry`, y2).attr(`x`, -v2.width - 4 - (n2.rotateCommitLabel === true ? 30 : 0)).attr(`y`, -v2.height / 2 + 10).attr(`width`, v2.width + 18 + b2).attr(`height`, v2.height + 4 + x2), _2.attr(`transform`, `translate(` + (-v2.width - 14 - (n2.rotateCommitLabel === true ? 30 : 0) + b2 / 2) + `, ` + (s3 - v2.height / 2 - 2) + `)`), K === `TB` ? (g2.attr(`x`, o2 - v2.width / 2 - 10).attr(`y`, 0), _2.attr(`transform`, `translate(` + (o2 - v2.width / 2 - 5) + `, 0)`), u2 && (g2.attr(`transform`, `translate(${-b2 / 2 - 3}, ${-x2 - 10})`), _2.attr(`transform`, `translate(` + (o2 - v2.width / 2 - 5) + `, ` + (-x2 * 2 + 7) + `)`))) : K === `BT` ? (g2.attr(`x`, o2 - v2.width / 2 - 10).attr(`y`, G), _2.attr(`transform`, `translate(` + (o2 - v2.width / 2 - 5) + `, ` + G + `)`), u2 && (g2.attr(`transform`, `translate(${-b2 / 2 - 3}, ${x2 + 10})`), _2.attr(`transform`, `translate(` + (o2 - v2.width / 2 - 5) + `, ` + (G + x2 * 2 + 4) + `)`))) : g2.attr(`transform`, `translate(-19, ` + (s3 - 12 - x2 / 2) + `)`);
  });
}, `drawBranches`), Re = p(function(e2, t2, n2, r2, i2) {
  return B.set(e2, { pos: t2, index: n2 }), t2 += 50 + (i2 ? 40 : 0) + (K === `TB` || K === `BT` ? r2.width / 2 : 0), t2;
}, `setBranchPosition`), ze = { draw: p(function(e2, t2, n2, r2) {
  q(), f.debug(`in gitgraph renderer`, e2 + `
`, `id:`, t2, n2);
  let i2 = r2.db;
  if (!i2.getConfig) {
  ...
  let a2 = i2.getConfig(), s2 = a2.rotateCommitLabel ?? false;
  U = i2.getCommits();
  let l2 = i2.getBranchesAsObjArray();
  K = i2.getDirection();
  let u2 = m(`[id="${t2}"]`), { look: d2, theme: p2, themeVariables: g2 } = o(), { useGradient: _2, gradientStart: v2, gradientStop: y2, filterColor: b2 } = g2;
  if (_2) {
    let e3 = u2.append(`defs`).append(`linearGradient`).attr(`id`, t2 + `-gradient`).attr(`gradientUnits`, `objectBoundingBox`).attr(`x1`, `0%`).attr(`y1`, `0%`).attr(`x2`, `100%`).attr(`y2`, `0%`);
    e3.append(`stop`).attr(`offset`, `0%`).attr(`stop-color`, v2).attr(`stop-opacity`, 1), e3.append(`stop`).attr(`offset`, `100%`).attr(`stop-color`, y2).attr(`stop-opacity`, 1);
  }
  d2 === `neo` && I.has(p2 ?? ``) && u2.append(`defs`).append(`filter`).attr(`id`, t2 + `-drop-shadow`).attr(`height`, `130%`).attr(`width`, `130%`).append(`feDropShadow`).attr(`dx`, `4`).attr(`dy`, `4`).attr(`stdDeviation`, 0).attr(`flood-opacity`, `0.06`).attr(`flood-color`, b2);
  let x2 = 0;
  l2.forEach((e3, t3) => {
    var _a;
    let n3 = J(e3.name), r3 = u2.append(`g`), i3 = r3.insert(`g`).attr(`class`, `branchLabel`), a3 = i3.insert(`g`).attr(`class`, `label branch-label`);
    (_a = a3.node()) == null ? void 0 : _a.appendChild(n3);
    let o2 = n3.getBBox();
    x2 = Re(e3.name, x2, t3, o2, s2), a3.remove(), i3.remove(), r3.remove();
  }), X(u2, U, false, a2), a2.showBranches && Le(u2, l2, a2, t2), Ie(u2, U), X(u2, U, true, a2), h.insertTitle(u2, `gitTitleText`, a2.titleTopMargin ?? 0, i2.getDiagramTitle()), c(void 0, u2, a2.diagramPadding, a2.useMaxWidth);
}, `draw`) }, Q = 8, $ = /* @__PURE__ */ new Set([`redux`, `redux-dark`, `redux-color`, `redux-dark-color`]), Be = /* @__PURE__ */ new Set([`redux-color`, `redux-dark-color`]), Ve = /* @__PURE__ */ new Set([`neo`, `neo-dark`]), He = /* @__PURE__ */ new Set([`dark`, `redux-dark`, `redux-dark-color`, `neo-dark`]), Ue = /* @__PURE__ */ new Set([`redux`, `redux-dark`, `redux-color`, `redux-dark-color`, `neo`, `neo-dark`]), We = p((e2) => {
  let { svgId: t2 } = e2, n2 = ``;
  if (e2.useGradient && t2) for (let r2 = 0; r2 < e2.THEME_COLOR_LIMIT; r2++) n2 += `
      .label${r2}  { fill: ${e2.mainBkg}; stroke: url(${t2}-gradient); stroke-width: ${e2.strokeWidth};}
             `;
  return n2;
}, `genGitGraphGradient`), Ge = p((e2) => {
  let { theme: t2, themeVariables: n2 } = d(), { borderColorArray: r2 } = n2, i2 = $.has(t2);
  if (Ve.has(t2)) {
    let t3 = ``;
...
}, `genColor`), Ke = p((e2) => `${Array.from({ length: e2.THEME_COLOR_LIMIT }, (e3, t2) => t2).map((t2) => {
  let n2 = t2 % Q;
  return `
...
`)}`, `normalTheme`), qe = { parser: be, db: A, renderer: ze, styles: p((e2) => {
  let { theme: t2 } = d(), n2 = Ue.has(t2);
  return `
  .commit-id,
  .commit-msg,
  .branch-label {
