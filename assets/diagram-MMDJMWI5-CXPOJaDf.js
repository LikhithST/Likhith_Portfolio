import { Cn as e, In as t, Jn as n, N as r, Nn as i, Nt as a, Pn as o, Tn as s, Yn as c, bn as l, gn as u, pn as d, vn as f, xn as p, yn as m } from "./index-4Ava25sD.js";
...
import { t as h } from "./chunk-4BX2VUAB-BsqOnTl0.js";
import { t as g } from "./mermaid-parser.core-C5_5uso8.js";
var _ = { showLegend: true, ticks: 5, max: null, min: 0, graticule: `circle` }, v = { axes: [], curves: [], options: _ }, y = structuredClone(v), b = f.radar, x = n(() => r({ ...b, ...p().radar }), `getConfig`), S = n(() => y.axes, `getAxes`), C = n(() => y.curves, `getCurves`), w = n(() => y.options, `getOptions`), T = n((e2) => {
...
}, `computeCurveEntries`), O = { getAxes: S, getCurves: C, getOptions: w, setAxes: T, setCurves: E, setOptions: n((e2) => {
  var _a, _b, _c, _d, _e;
...
}, `setOptions`), getConfig: x, clear: n(() => {
  d(), y = structuredClone(v);
}, `clear`), setAccTitle: o, getAccTitle: l, setDiagramTitle: t, getDiagramTitle: e, getAccDescription: m, setAccDescription: i }, k = n((e2) => {
  h(e2, O);
  let { axes: t2, curves: n2, options: r2 } = e2;
  O.setAxes(t2), O.setCurves(n2), O.setOptions(r2);
}, `populate`), A = { parse: n(async (e2) => {
  let t2 = await g(`radar`, e2);
  c.debug(t2), k(t2);
}, `parse`) }, j = n((e2, t2, n2, r2) => {
  let i2 = r2.db, o2 = i2.getAxes(), s2 = i2.getCurves(), c2 = i2.getOptions(), l2 = i2.getConfig(), u2 = i2.getDiagramTitle(), d2 = M(a(t2), l2), f2 = c2.max ?? Math.max(...s2.map((e3) => Math.max(...e3.entries))), p2 = c2.min, m2 = Math.min(l2.width, l2.height) / 2;
  N(d2, o2, m2, c2.ticks, c2.graticule), P(d2, o2, m2, l2), F(d2, o2, s2, p2, f2, c2.graticule, l2), R(d2, s2, c2.showLegend, l2), d2.append(`text`).attr(`class`, `radarTitle`).text(u2).attr(`x`, 0).attr(`y`, -l2.height / 2 - l2.marginTop);
}, `draw`), M = n((e2, t2) => {
  ...
  return u(e2, r2, n2, t2.useMaxWidth ?? true), e2.attr(`viewBox`, `0 0 ${n2} ${r2}`), e2.append(`g`).attr(`transform`, `translate(${i2.x}, ${i2.y})`);
}, `drawFrame`), N = n((e2, t2, n2, r2, i2) => {
...
var z = { draw: j }, B = n((e2, t2) => {
  ...
  return n2;
}, `genIndexStyles`), V = n((e2) => {
  let t2 = r(s(), p().themeVariables);
  return { themeVariables: t2, radarOptions: r(t2.radar, e2) };
}, `buildRadarStyleOptions`), H = { parser: A, db: O, renderer: z, styles: n(({ radar: e2 } = {}) => {
