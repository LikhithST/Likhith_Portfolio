import { f as Gt, b1 as Wt, a2 as Kt, a7 as Ht, b4 as Yt, a3 as Xt, b2 as Zt, b as w, a4 as wt, b0 as Jt, a$ as ot, m as Qt, aw as $t, aB as te, M as ee, F as ne, u as se, t as ie, T as oe } from "./index-ZamPsWkW.js";
...
function st(t, n) {
  ...
  if (e.length > 1) {
    ...
    for (let u = 0; u < e.length; ++u) {
      ...
      let d = null;
      for (let b = 0; b < r.parentIndex.length; ++b) if (h.parentIndex.includes(r.parentIndex[b])) {
        ...
        g > x.radius * 2 && (g = x.radius * 2), (d == null || d.width > g) && (d = { circle: x, width: g, p1: r, p2: h, large: g > x.radius, sweep: true });
      }
    ...
    h ? i = o = 0 : (i = c.radius * c.radius * Math.PI, a.push({ circle: c, p1: { x: c.x, y: c.y + c.radius }, p2: { x: c.x - _t, y: c.y + c.radius }, width: c.radius * 2, large: true, sweep: true }));
  }
...
function Tt(t, n) {
  ...
  return [{ x: c + u, y: h - r }, { x: c - u, y: h + r }];
}
function Et(t) {
  const n = { x: 0, y: 0 };
...
function zt(t, n, s) {
  ...
  for (let v = 0; v < e; ++v) {
    ...
    const l = x[b];
    if (J(S, 1 + h, T, -h, l), S.fx = t(S), S.fx < x[0].fx) J(m, 1 + u, T, -u, l), m.fx = t(m), m.fx < S.fx ? M(m) : M(S);
    else if (S.fx >= x[b - 1].fx) {
      let p = false;
      if (S.fx > l.fx ? (J(g, 1 + r, T, -r, l), g.fx = t(g), g.fx < l.fx ? M(g) : p = true) : (J(g, 1 - r * h, T, r * h, l), g.fx = t(g), g.fx < S.fx ? M(g) : p = true), p) {
        if (y >= 1) break;
...
function ce(t, n, s, e, i, o, a) {
  ...
  function b(x, M, A) {
    for (let T = 0; T < 16; ++T) if (i = (x + M) / 2, J(e.x, 1, s.x, i, n), u = e.fx = t(e.x, e.fxprime), y = $(e.fxprime, n), u > c + o * i * h || u >= A) M = i;
    else {
  ...
  for (let x = 0; x < 10; ++x) {
    if (J(e.x, 1, s.x, i, n), u = e.fx = t(e.x, e.fxprime), y = $(e.fxprime, n), u > c + o * i * h || x && u >= r) return b(d, i, r);
    if (Math.abs(y) <= -a * h) return i;
...
function ue(t, n, s) {
  ...
  for (let r = 0; r < u; ++r) {
    if (h = ce(t, a, e, i, h), s.history && s.history.push({ x: e.x.slice(), fx: e.fx, fxprime: e.fxprime.slice(), alpha: h }), !h) ft(a, e.fxprime, -1);
    else {
...
function fe(t, n = {}) {
  ...
  if (s) {
    ...
    for (const h of e) for (let u = 0; u < h.sets.length; u++) {
  ...
  for (const c of e) if (c.sets.length === 1) o.push(c.sets[0]);
...
function ye(t, n) {
  ...
  for (const y of t) if (y.sets.length === 1) {
    const d = y.sets[0];
  ...
  for (let y = 1; y < o.length; ++y) {
    ...
    for (var r = 0; r < b.length; ++r) {
      ...
      for (let m = r + 1; m < b.length; ++m) {
        const v = e[b[m].set], l = ht(x.radius, v.radius, b[m].size), p = Tt({ x: S.x, y: S.y, radius: g }, { x: v.x, y: v.y, radius: l });
        M.push(...p);
      }
    }
    let A = 1e50, T = M[0];
...
function Dt(t, n, s) {
  ...
  let h = 1;
  for (; h < i.length; ) c(i[h], true, false), c(i[h + 1], false, true), c(i[h + 2], true, true), h += 3, o = dt(e);
  return Ot(e);
}
function Nt(t, n, s, e, i) {
  ...
  return Ot(o.map((b) => ({ radius: r * b.radius, x: e + y + (b.x - a.min) * r, y: e + d + (b.y - c.min) * r, setid: b.setid })));
}
function Ot(t) {
  const n = {};
...
function be(t = {}) {
  let n = false, s = 600, e = 350, i = 15, o = 1e3, a = Math.PI / 2, c = true, h = null, u = true, r = true, y = null, d = null, b = false, x = null, M = t && t.symmetricalTextCentre ? t.symmetricalTextCentre : false, A = {}, T = t && t.colourScheme ? t.colourScheme : t && t.colorScheme ? t.colorScheme : ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"], S = 0, g = function(p) {
  ...
  function l(p) {
    ...
    E.selectAll(".venn-area path").each(function(k) {
      ...
      k.sets.length == 1 && F && !b && (_ = true, R[k.sets[0]] = ke(F));
    });
    function P(k) {
      return (F) => {
        const H = k.sets.map((et) => {
          let Y = R[et], Z = I[et];
          return Y || (Y = { x: s / 2, y: e / 2, radius: 1 }), Z || (Z = { x: s / 2, y: e / 2, radius: 1 }), { x: Y.x * (1 - F) + Z.x * F, y: Y.y * (1 - F) + Z.y * F, radius: Y.radius * (1 - F) + Z.radius * F };
        });
        return St(H, x);
      };
    }
    const B = E.selectAll(".venn-area").data(f, (k) => k.sets), O = B.enter().append("g").attr("class", (k) => `venn-area venn-${k.sets.length == 1 ? "circle" : "intersection"}${k.colour || k.color ? " venn-coloured" : ""}`).attr("data-venn-sets", (k) => k.sets.join("_")), V = O.append("path"), W = O.append("text").attr("class", "label").text((k) => j(k)).attr("text-anchor", "middle").attr("dy", ".35em").attr("x", s / 2).attr("y", e / 2);
    r && (V.style("fill-opacity", "0").filter((k) => k.sets.length == 1).style("fill", (k) => k.colour ? k.colour : k.color ? k.color : g(k.sets)).style("fill-opacity", ".25"), W.style("fill", (k) => k.colour || k.color ? "#FFF" : t.textFill ? t.textFill : k.sets.length == 1 ? g(k.sets) : "#444"));
    function K(k) {
    ...
    const D = K(B.exit()).remove();
    typeof B.transition == "function" && D.selectAll("path").attrTween("d", P);
    const X = D.selectAll("text").attr("x", s / 2).attr("y", e / 2);
    return y !== null && (W.style("font-size", "0px"), L.style("font-size", y), X.style("font-size", "0px")), { circles: I, textCentres: C, nodes: B, enter: O, update: z, exit: D };
  }
...
function Lt(t, n, s) {
  const e = [];
  for (const r of t) e.push({ x: r.x, y: r.y }), e.push({ x: r.x + r.radius / 2, y: r.y }), e.push({ x: r.x - r.radius / 2, y: r.y }), e.push({ x: r.x, y: r.y + r.radius / 2 }), e.push({ x: r.x, y: r.y - r.radius / 2 });
  let i = e[0], o = at(e[0], t, n);
  for (let r = 1; r < e.length; ++r) {
    const y = at(e[r], t, n);
    y >= o && (i = e[r], o = y);
  }
  const a = zt((r) => -1 * at({ x: r[0], y: r[1] }, t, n), [i.x, i.y], { maxIterations: 500, minErrorDelta: 1e-10 }).x, c = { x: s ? 0 : a[0], y: a[1] };
  let h = true;
  for (const r of t) if (q(c, r) > r.radius) {
    h = false;
    break;
  }
  for (const r of n) if (q(c, r) < r.radius) {
    h = false;
    break;
  }
  if (h) return c;
  ...
  const u = {};
  return st(t, u), u.arcs.length === 0 ? { x: 0, y: -1e3, disjoint: true } : u.arcs.length == 1 ? { x: u.arcs[0].circle.x, y: u.arcs[0].circle.y } : n.length ? Lt(t, []) : Et(u.arcs.map((r) => r.p1));
}
function ve(t) {
...
function jt(t, n, s) {
  ...
  for (let o = 0; o < n.length; ++o) {
    ...
    for (let d = 0; d < a.length; ++d) {
      c[a[d]] = true;
      const b = i[a[d]];
      for (let x = 0; x < b.length; ++x) h[b[x]] = true;
    }
    const u = [], r = [];
...
function Me(t, n = {}) {
  const { lossFunction: s, layoutFunction: e = At, normalize: i = true, orientation: o = Math.PI / 2, orientationOrder: a, width: c = 600, height: h = 350, padding: u = 15, scaleToFit: r = false, symmetricalTextCentre: y = false, distinct: d, round: b = 2 } = n;
  let x = e(t, { lossFunction: s === "default" || !s ? tt : s === "logRatio" ? Ct : s, distinct: d });
  i && (x = Dt(x, o, a));
  const M = Nt(x, c, h, u, r), A = jt(M, t, y), T = new Map(Object.keys(M).map((m) => [m, { set: m, x: M[m].x, y: M[m].y, radius: M[m].radius }])), S = t.map((m) => {
    const v = m.sets.map((f) => T.get(f)), l = Pt(v), p = Bt(l, b);
  ...
  return S.map(({ circles: m, arcs: v, path: l, area: p }) => ({ data: p, text: A[p.sets], circles: m, arcs: v, path: l, distinctPath: l + g(p.sets) }));
}
var gt = (function() {
  var t = w(function(S, g, m, v) {
    for (m = m || {}, v = S.length; v--; m[S[v]] = g) ;
    return m;
  }, "o"), n = [5, 8], s = [7, 8, 11, 12, 17, 19, 22, 24], e = [1, 17], i = [1, 18], o = [7, 8, 11, 12, 14, 15, 16, 17, 19, 20, 21, 22, 24, 27], a = [1, 31], c = [1, 39], h = [7, 8, 11, 12, 17, 19, 22, 24, 27], u = [1, 57], r = [1, 56], y = [1, 58], d = [1, 59], b = [1, 60], x = [7, 8, 11, 12, 16, 17, 19, 20, 22, 24, 27, 31, 32, 33], M = { trace: w(function() {
  }, "trace"), yy: {}, symbols_: { error: 2, start: 3, optNewlines: 4, VENN: 5, document: 6, EOF: 7, NEWLINE: 8, line: 9, statement: 10, TITLE: 11, SET: 12, identifier: 13, BRACKET_LABEL: 14, COLON: 15, NUMERIC: 16, UNION: 17, identifierList: 18, TEXT: 19, IDENTIFIER: 20, STRING: 21, INDENT_TEXT: 22, indentedTextTail: 23, STYLE: 24, stylesOpt: 25, styleField: 26, COMMA: 27, styleValue: 28, valueTokens: 29, valueToken: 30, HEXCOLOR: 31, RGBCOLOR: 32, RGBACOLOR: 33, $accept: 0, $end: 1 }, terminals_: { 2: "error", 5: "VENN", 7: "EOF", 8: "NEWLINE", 11: "TITLE", 12: "SET", 14: "BRACKET_LABEL", 15: "COLON", 16: "NUMERIC", 17: "UNION", 19: "TEXT", 20: "IDENTIFIER", 21: "STRING", 22: "INDENT_TEXT", 24: "STYLE", 27: "COMMA", 31: "HEXCOLOR", 32: "RGBCOLOR", 33: "RGBACOLOR" }, productions_: [0, [3, 4], [4, 0], [4, 2], [6, 0], [6, 2], [9, 1], [9, 1], [10, 1], [10, 2], [10, 3], [10, 4], [10, 5], [10, 2], [10, 3], [10, 4], [10, 5], [10, 3], [10, 3], [10, 3], [10, 4], [10, 4], [10, 2], [10, 3], [23, 1], [23, 1], [23, 1], [23, 2], [23, 2], [25, 1], [25, 3], [26, 3], [28, 1], [28, 1], [29, 1], [29, 2], [30, 1], [30, 1], [30, 1], [30, 1], [30, 1], [18, 1], [18, 3], [13, 1], [13, 1]], performAction: w(function(g, m, v, l, p, f, N) {
    var I = f.length - 1;
    switch (p) {
      case 1:
        return f[I - 1];
    ...
    var O = _.yylloc;
    f.push(O);
    var V = _.options && _.options.ranges;
...
` + _.showPosition() + `
Expecting ` + Y.join(", ") + ", got '" + (this.terminals_[z] || z) + "'" : Z = "Parse error on line " + (C + 1) + ": Unexpected " + (z == E ? "end of input" : "'" + (this.terminals_[z] || z) + "'"), this.parseError(Z, { text: _.match, token: this.terminals_[z] || z, line: _.yylineno, loc: O, expected: Y });
      }
      if (D[0] instanceof Array && D.length > 1) throw new Error("Parse Error: multiple actions possible at state: " + L + ", token: " + z);
      switch (D[0]) {
        case 1:
          v.push(z), p.push(_.yytext), f.push(_.yylloc), v.push(D[1]), z = null, U = _.yyleng, I = _.yytext, C = _.yylineno, O = _.yylloc;
          break;
        case 2:
          if (H = this.productions_[D[1]][1], k.$ = p[p.length - H], k._$ = { first_line: f[f.length - (H || 1)].first_line, last_line: f[f.length - 1].last_line, first_column: f[f.length - (H || 1)].first_column, last_column: f[f.length - 1].last_column }, V && (k._$.range = [f[f.length - (H || 1)].range[0], f[f.length - 1].range[1]]), X = this.performAction.apply(k, [I, U, C, P.yy, D[1], p, f].concat(R)), typeof X < "u") return X;
    ...
    var S = { EOF: 1, parseError: w(function(m, v) {
      if (this.yy.parser) this.yy.parser.parseError(m, v);
      else throw new Error(m);
    }, "parseError"), setInput: w(function(g, m) {
      return this.yy = m || this.yy || {}, this._input = g, this._more = this._backtrack = this.done = false, this.yylineno = this.yyleng = 0, this.yytext = this.matched = this.match = "", this.conditionStack = ["INITIAL"], this.yylloc = { first_line: 1, first_column: 0, last_line: 1, last_column: 0 }, this.options.ranges && (this.yylloc.range = [0, 0]), this.offset = 0, this;
    }, "setInput"), input: w(function() {
      var g = this._input[0];
      ...
      return this.yylloc = { first_line: this.yylloc.first_line, last_line: this.yylineno + 1, first_column: this.yylloc.first_column, last_column: v ? (v.length === l.length ? this.yylloc.first_column : 0) + l[l.length - v.length].length - v[0].length : this.yylloc.first_column - m }, this.options.ranges && (this.yylloc.range = [p[0], p[0] + this.yyleng - m]), this.yyleng = this.yytext.length, this;
    }, "unput"), more: w(function() {
      return this._more = true, this;
    }, "more"), reject: w(function() {
      if (this.options.backtrack_lexer) this._backtrack = true;
      else return this.parseError("Lexical error on line " + (this.yylineno + 1) + `. You can only invoke reject() in the lexer when the lexer is of the backtracking persuasion (options.backtrack_lexer = true).
` + this.showPosition(), { text: "", token: null, line: this.yylineno });
      return this;
    }, "reject"), less: w(function(g) {
      this.unput(this.match.slice(g));
    }, "less"), pastInput: w(function() {
      var g = this.matched.substr(0, this.matched.length - this.match.length);
      return (g.length > 20 ? "..." : "") + g.substr(-20).replace(/\n/g, "");
    }, "pastInput"), upcomingInput: w(function() {
      var g = this.match;
      return g.length < 20 && (g += this._input.substr(0, 20 - g.length)), (g.substr(0, 20) + (g.length > 20 ? "..." : "")).replace(/\n/g, "");
    }, "upcomingInput"), showPosition: w(function() {
      var g = this.pastInput(), m = new Array(g.length + 1).join("-");
      return g + this.upcomingInput() + `
` + m + "^";
    }, "showPosition"), test_match: w(function(g, m) {
      var v, l, p;
      if (this.options.backtrack_lexer && (p = { yylineno: this.yylineno, yylloc: { first_line: this.yylloc.first_line, last_line: this.last_line, first_column: this.yylloc.first_column, last_column: this.yylloc.last_column }, yytext: this.yytext, match: this.match, matches: this.matches, matched: this.matched, yyleng: this.yyleng, offset: this.offset, _more: this._more, _input: this._input, yy: this.yy, conditionStack: this.conditionStack.slice(0), done: this.done }, this.options.ranges && (p.yylloc.range = this.yylloc.range.slice(0))), l = g[0].match(/(?:\r\n?|\n).*/g), l && (this.yylineno += l.length), this.yylloc = { first_line: this.yylloc.last_line, last_line: this.yylineno + 1, first_column: this.yylloc.last_column, last_column: l ? l[l.length - 1].length - l[l.length - 1].match(/\r?\n?/)[0].length : this.yylloc.last_column + g[0].length }, this.yytext += g[0], this.match += g[0], this.matches = g, this.yyleng = this.yytext.length, this.options.ranges && (this.yylloc.range = [this.offset, this.offset += this.yyleng]), this._more = false, this._backtrack = false, this._input = this._input.slice(g[0].length), this.matched += g[0], v = this.performAction.call(this, this.yy, this, m, this.conditionStack[this.conditionStack.length - 1]), this.done && this._input && (this.done = false), v) return v;
      if (this._backtrack) {
        for (var f in p) this[f] = p[f];
        return false;
      }
      return false;
    }, "test_match"), next: w(function() {
      if (this.done) return this.EOF;
      this._input || (this.done = true);
      var g, m, v, l;
      this._more || (this.yytext = "", this.match = "");
      for (var p = this._currentRules(), f = 0; f < p.length; f++) if (v = this._input.match(this.rules[p[f]]), v && (!m || v[0].length > m[0].length)) {
        if (m = v, l = f, this.options.backtrack_lexer) {
          if (g = this.test_match(v, p[f]), g !== false) return g;
          if (this._backtrack) {
            m = false;
            continue;
          } else return false;
      ...
      return m ? (g = this.test_match(m, p[l]), g !== false ? g : false) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), { text: "", token: null, line: this.yylineno });
    }, "next"), lex: w(function() {
      var m = this.next();
      return m || this.lex();
    }, "lex"), begin: w(function(m) {
      this.conditionStack.push(m);
    }, "begin"), popState: w(function() {
      var m = this.conditionStack.length - 1;
      return m > 0 ? this.conditionStack.pop() : this.conditionStack[0];
    }, "popState"), _currentRules: w(function() {
      return this.conditionStack.length && this.conditionStack[this.conditionStack.length - 1] ? this.conditions[this.conditionStack[this.conditionStack.length - 1]].rules : this.conditions.INITIAL.rules;
    }, "_currentRules"), topState: w(function(m) {
      return m = this.conditionStack.length - 1 - Math.abs(m || 0), m >= 0 ? this.conditionStack[m] : "INITIAL";
    }, "topState"), pushState: w(function(m) {
      this.begin(m);
    }, "pushState"), stateStackSize: w(function() {
      return this.conditionStack.length;
    }, "stateStackSize"), options: { "case-insensitive": true }, performAction: w(function(m, v, l, p) {
      switch (l) {
        case 0:
          break;
        case 1:
        ...
        case 2:
          break;
        case 3:
          if (m.getIndentMode && m.getIndentMode()) return m.consumeIndentText = true, this.begin("INITIAL"), 22;
          break;
        case 4:
          break;
        case 5:
          m.setIndentMode && m.setIndentMode(false), this.begin("INITIAL"), this.unput(v.yytext);
          break;
        case 6:
        ...
        case 13:
          return 17;
        case 14:
          if (m.consumeIndentText) m.consumeIndentText = false;
          else return 19;
          break;
        case 15:
          return 24;
...
gt.parser = gt;
var Se = gt, yt = [], pt = [], mt = [], bt = /* @__PURE__ */ new Set(), vt, It = false, we = w((t, n, s) => {
  const e = it(t).sort(), i = s ?? 10 / Math.pow(t.length, 2);
  vt = e, e.length === 1 && bt.add(e[0]), yt.push({ sets: e, size: i, label: n ? nt(n) : void 0 });
}, "addSubsetData"), _e = w(() => yt, "getSubsetData"), nt = w((t) => {
  const n = t.trim();
  return n.length >= 2 && n.startsWith('"') && n.endsWith('"') ? n.slice(1, -1) : n;
}, "normalizeText"), Te = w((t) => t && nt(t), "normalizeStyleValue"), Ee = w((t, n, s) => {
  const e = nt(n);
  pt.push({ sets: it(t).sort(), id: e, label: s ? nt(s) : void 0 });
}, "addTextData"), ze = w((t, n) => {
  const s = it(t).sort(), e = {};
  for (const [i, o] of n) e[i] = Te(o) ?? o;
  mt.push({ targets: s, styles: e });
}, "addStyleData"), Ae = w(() => mt, "getStyleData"), it = w((t) => t.map((n) => nt(n)), "normalizeIdentifierList"), Re = w((t) => {
  const s = it(t).filter((e) => !bt.has(e));
  if (s.length > 0) throw new Error(`unknown set identifier: ${s.join(", ")}`);
}, "validateUnionIdentifiers"), Ce = w(() => pt, "getTextData"), De = w(() => vt, "getCurrentSets"), Ne = w(() => It, "getIndentMode"), Oe = w((t) => {
  It = t;
...
w(Vt, "getConfig");
var Le = w(() => {
  se(), yt.length = 0, pt.length = 0, mt.length = 0, bt.clear(), vt = void 0, It = false;
}, "customClear"), je = { getConfig: Vt, clear: Le, setAccTitle: Zt, getAccTitle: Xt, setDiagramTitle: Yt, getDiagramTitle: Ht, getAccDescription: Kt, setAccDescription: Wt, addSubsetData: we, getSubsetData: _e, addTextData: Ee, addStyleData: ze, validateUnionIdentifiers: Re, getTextData: Ce, getStyleData: Ae, getCurrentSets: De, getIndentMode: Ne, setIndentMode: Oe }, Pe = w((t) => `
  .venn-title {
    font-size: 32px;
...
var Ve = w((t, n, s, e) => {
  var _a, _b, _c;
  const i = e.db, o = (_a = i.getConfig) == null ? void 0 : _a.call(i), { themeVariables: a, look: c, handDrawnSeed: h } = wt(), u = c === "handDrawn", r = [a.venn1, a.venn2, a.venn3, a.venn4, a.venn5, a.venn6, a.venn7, a.venn8].filter(Boolean), y = (_b = i.getDiagramTitle) == null ? void 0 : _b.call(i), d = i.getSubsetData(), b = i.getTextData(), x = qt(i.getStyleData()), M = (o == null ? void 0 : o.width) ?? 800, A = (o == null ? void 0 : o.height) ?? 450, S = M / 1600, g = y ? 48 * S : 0, m = a.primaryTextColor ?? a.textColor, v = Jt(n);
  ...
  l.datum(d).call(p);
  const f = u ? Qt.svg(l.select("svg").node()) : void 0, N = Me(d, { width: M, height: A - g, padding: (o == null ? void 0 : o.padding) ?? 15 }), I = /* @__PURE__ */ new Map();
  for (const E of N) {
  ...
  l.selectAll(".venn-circle").each(function(E, R) {
    var _a2;
    const _ = ot(this), B = Q([...E.sets].sort()), O = x.get(B), V = (O == null ? void 0 : O.fill) || r[R % r.length] || a.primaryColor;
    _.classed(`venn-set-${R % 8}`, true);
    const W = (O == null ? void 0 : O["fill-opacity"]) ?? 0.1, K = (O == null ? void 0 : O.stroke) || V, z = (O == null ? void 0 : O["stroke-width"]) || `${5 * S}`;
    if (u && f) {
      const D = I.get(B);
      if (D && D.circles.length > 0) {
        const X = D.circles[0], k = f.circle(X.x, X.y, X.radius * 2, { roughness: 0.7, seed: h, fill: kt(V, 0.7), fillStyle: "hachure", fillWeight: 2, hachureGap: 8, hachureAngle: -41 + R * 60, stroke: K, strokeWidth: parseFloat(String(z)) });
        _.select("path").remove(), (_a2 = _.node()) == null ? void 0 : _a2.insertBefore(k, _.select("text").node());
      }
    } else _.select("path").style("fill", V).style("fill-opacity", W).style("stroke", K).style("stroke-width", z).style("stroke-opacity", 0.95);
    const L = (O == null ? void 0 : O.color) || (C ? te(V, 30) : ee(V, 30));
    _.select("text").style("font-size", `${48 * S}px`).style("fill", L);
  }), u && f ? l.selectAll(".venn-intersection").each(function(E) {
    var _a2;
    const R = ot(this), P = Q([...E.sets].sort()), B = x.get(P), O = B == null ? void 0 : B.fill;
    if (O) {
      const V = R.select("path"), W = V.attr("d");
      if (W) {
        const K = f.path(W, { roughness: 0.7, seed: h, fill: kt(O, 0.3), fillStyle: "cross-hatch", fillWeight: 2, hachureGap: 6, hachureAngle: 60, stroke: "none" }), z = V.node();
        (_a2 = z == null ? void 0 : z.parentNode) == null ? void 0 : _a2.insertBefore(K, z), V.remove();
      }
    } else R.select("path").style("fill-opacity", 0);
    R.select("text").style("font-size", `${48 * S}px`).style("fill", (B == null ? void 0 : B.color) ?? a.vennSetTextColor ?? m);
  }) : (l.selectAll(".venn-intersection text").style("font-size", `${48 * S}px`).style("fill", (E) => {
    var _a2;
    const _ = Q([...E.sets].sort());
    return ((_a2 = x.get(_)) == null ? void 0 : _a2.color) ?? a.vennSetTextColor ?? m;
  }), l.selectAll(".venn-intersection path").style("fill-opacity", (E) => {
    var _a2;
    const _ = Q([...E.sets].sort());
    return ((_a2 = x.get(_)) == null ? void 0 : _a2.fill) ? 1 : 0;
  }).style("fill", (E) => {
    var _a2;
    const _ = Q([...E.sets].sort());
    return ((_a2 = x.get(_)) == null ? void 0 : _a2.fill) ?? "transparent";
  }));
  const U = v.append("g").attr("transform", `translate(0, ${g})`), j = l.select("svg").node();
  if (j && "childNodes" in j) for (const E of [...j.childNodes]) (_c = U.node()) == null ? void 0 : _c.appendChild(E);
  ne(v, A, M, (o == null ? void 0 : o.useMaxWidth) ?? true);
}, "draw");
function Q(t) {
  return t.join("|");
}
...
function Ut(t, n, s, e, i, o) {
  var _a;
  const a = (t == null ? void 0 : t.useDebugLayout) ?? false, h = s.select("svg").append("g").attr("class", "venn-text-nodes"), u = /* @__PURE__ */ new Map();
  for (const r of e) {
  ...
  for (const [r, y] of u.entries()) {
    const d = n.get(r);
    if (!(d == null ? void 0 : d.text)) continue;
    const b = d.text.x, x = d.text.y, M = Math.min(...d.circles.map((E) => E.radius)), A = Math.min(...d.circles.map((E) => E.radius - Math.hypot(b - E.x, x - E.y)));
    ...
    for (const [E, R] of y.entries()) {
      const _ = E % I, P = Math.floor(E / I), B = f + U * (_ + 0.5), O = N + j * (P + 0.5);
