...
import { B as e, C as t, E as n, L as r, V as i, W as a, _ as o, a as s, c, d as l, v as u, y as d } from "./chunk-ICPOFSXX-qplCg4jO.js";
import { g as f, h as p } from "./src-BWB47EtT.js";
import { t as m } from "./linear-CWOc3ypC.js";
import { t as h } from "./ordinal-BV8Hym1O.js";
import "./path-CAD_RBb9.js";
import "./defaultLocale-DPK_2BZd.js";
import { t as g } from "./init-BNJjXmOc.js";
import "./dist-DAHuMNYw.js";
import "./array-BsT2bq1e.js";
import { i as _ } from "./chunk-5PVQY5BW-DyH3Wf6D.js";
import { t as v } from "./line-CKQlCIK2.js";
import { t as y } from "./chunk-426QAEUC-BhmuD47R.js";
import "./identity-B9owP9ru.js";
import { t as b } from "./chunk-U2HBQHQK-B952OdWg.js";
function x(e2, t2, n2) {
...
var _e = { parser: w, db: { getDrawableElem: me, clear: p(function() {
  s(), F = 0, L = W(), z = G(), R = U(), B = R.plotColorPalette.split(`,`).map((e2) => e2.trim()), V = false, H = false;
}, `clear`), setAccTitle: i, getAccTitle: u, setDiagramTitle: a, getDiagramTitle: t, getAccDescription: o, setAccDescription: e, setOrientation: J, setXAxisTitle: Y, setXAxisRangeData: X, setXAxisBand: ce, setYAxisTitle: le, setYAxisRangeData: ue, setLineData: fe, setBarData: pe, setTmpSVGG: q, getChartThemeConfig: he, getChartConfig: ge, getXYChartData: $ }, renderer: { draw: p((e2, t2, n2, r2) => {
  let i2 = r2.db, a2 = i2.getChartThemeConfig(), o2 = i2.getChartConfig(), s2 = i2.getXYChartData().plots[0].data.map((e3) => e3[1]);
  function l2(e3) {
...
` + e2);
  ...
  c(m2, o2.height, o2.width, true), m2.attr(`viewBox`, `0 0 ${o2.width} ${o2.height}`), g2.attr(`fill`, a2.backgroundColor), i2.setTmpSVGG(m2.append(`g`).attr(`class`, `mermaid-tmp-group`));
  ...
  for (let e3 of _2) {
    ...
    switch (e3.type) {
      case `rect`:
        if (t3.selectAll(`rect`).data(e3.data).enter().append(`rect`).attr(`x`, (e4) => e4.x).attr(`y`, (e4) => e4.y).attr(`width`, (e4) => e4.width).attr(`height`, (e4) => e4.height).attr(`fill`, (e4) => e4.fill).attr(`stroke`, (e4) => e4.strokeFill).attr(`stroke-width`, (e4) => e4.strokeWidth), o2.showDataLabel) {
          let n3 = o2.showDataLabelOutsideBar;
          if (o2.chartOrientation === `horizontal`) {
            let r3 = function(e4, t4) {
              let { data: n4, label: r4 } = e4;
              return t4 * r4.length * i3 <= n4.width - o3;
            };
            p(r3, `fitsHorizontally`);
            let i3 = 0.7, o3 = 10, c2 = e3.data.map((e4, t4) => ({ data: e4, label: s2[t4].toString() })).filter((e4) => e4.data.width > 0 && e4.data.height > 0), l3 = c2.map((e4) => {
              let { data: t4 } = e4, n4 = t4.height * 0.7;
              for (; !r3(e4, n4) && n4 > 0; ) --n4;
              return n4;
            }), u3 = Math.floor(Math.min(...l3)), d3 = p((e4) => n3 ? e4.data.x + e4.data.width + 10 : e4.data.x + e4.data.width - 10, `determineLabelXPosition`);
            t3.selectAll(`text`).data(c2).enter().append(`text`).attr(`x`, d3).attr(`y`, (e4) => e4.data.y + e4.data.height / 2).attr(`text-anchor`, n3 ? `start` : `end`).attr(`dominant-baseline`, `middle`).attr(`fill`, a2.dataLabelColor).attr(`font-size`, `${u3}px`).text((e4) => e4.label);
          } else {
            let r3 = function(e4, t4, n4) {
              let { data: r4, label: i4 } = e4, a3 = t4 * i4.length * 0.7, o4 = r4.x + r4.width / 2, s3 = o4 - a3 / 2, c3 = o4 + a3 / 2, l4 = s3 >= r4.x && c3 <= r4.x + r4.width, u3 = r4.y + n4 + t4 <= r4.y + r4.height;
              return l4 && u3;
            };
            p(r3, `fitsInBar`);
            let i3 = e3.data.map((e4, t4) => ({ data: e4, label: s2[t4].toString() })).filter((e4) => e4.data.width > 0 && e4.data.height > 0), o3 = i3.map((e4) => {
              let { data: t4, label: n4 } = e4, i4 = t4.width / (n4.length * 0.7);
              for (; !r3(e4, i4, 10) && i4 > 0; ) --i4;
              return i4;
            }), c2 = Math.floor(Math.min(...o3)), l3 = p((e4) => n3 ? e4.data.y - 10 : e4.data.y + 10, `determineLabelYPosition`);
            t3.selectAll(`text`).data(i3).enter().append(`text`).attr(`x`, (e4) => e4.data.x + e4.data.width / 2).attr(`y`, l3).attr(`text-anchor`, `middle`).attr(`dominant-baseline`, n3 ? `auto` : `hanging`).attr(`fill`, a2.dataLabelColor).attr(`font-size`, `${c2}px`).text((e4) => e4.label);
          }
        }
        break;
      case `text`:
