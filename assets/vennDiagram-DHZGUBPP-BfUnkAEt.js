import { Bn as darken_default, Cn as getDiagramTitle, Hn as adjust_channel_default, In as setDiagramTitle, Jn as __name, N as cleanAndMerge, Nn as setAccDescription, Nt as selectSvgElement, Pn as setAccTitle, Un as is_dark_default, Vn as lighten_default, b as at, bn as getAccTitle, gn as configureSvgSize, pn as clear, un as select_default, vn as defaultConfig_default, xn as getConfig, yn as getAccDescription } from "./index-BgJKKt_a.js";
...
var draw = /* @__PURE__ */ __name((_text, id, _version, diagObj) => {
  ...
  dummyD3root.selectAll(".venn-circle").each(function(d, i) {
    ...
    const strokeWidthVal = (customStyle == null ? void 0 : customStyle["stroke-width"]) || `${5 * scale$1}`;
    ...
    } else group.select("path").style("fill", baseColor).style("fill-opacity", fillOpacity).style("stroke", strokeColor).style("stroke-width", strokeWidthVal).style("stroke-opacity", 0.95);
  ...
  if (isHandDrawn && roughSvg) dummyD3root.selectAll(".venn-intersection").each(function(d) {
    ...
    group.select("text").style("font-size", `${48 * scale$1}px`).style("fill", (customStyle == null ? void 0 : customStyle.color) ?? themeVariables.vennSetTextColor ?? defaultTextColor);
  });
  else {
  ...
  if (dummySvg && "childNodes" in dummySvg) for (const child of [...dummySvg.childNodes]) (_c = vennGroup.node()) == null ? void 0 : _c.appendChild(child);
...
function renderTextNodes(config, layoutByKey, dummyD3root, textNodes2, scale$1, styleByKey) {
  ...
  for (const [key, nodes] of nodesByArea.entries()) {
    ...
    if (!(area == null ? void 0 : area.text)) continue;
    ...
    if (innerRadius === 0 && Number.isFinite(minCircleRadius)) innerRadius = minCircleRadius * 0.6;
    ...
    const innerWidth = Math.max(80 * scale$1, innerRadius * 2 * 0.95);
    const innerHeight = Math.max(60 * scale$1, innerRadius * 2 * 0.95);
    const labelOffset = (area.data.label && area.data.label.length > 0 ? Math.min(32 * scale$1, innerRadius * 0.25) : 0) + (nodes.length <= 2 ? 30 * scale$1 : 0);
